...
### 2. Ship Game IQ 101
It is built, authored, and animated but invisible. Run a full authoring audit across the 114 situations (starting positions, alignment correctness, sport-respective language for baseball vs softball), fix what the audit flags, then lift the lock: free tier gets a sample set, subscribers get the full module, owner keeps authoring tools. Progress and mastery flow into the athlete's roadmap like every other module.
...
- Game IQ unlock is a change to `GameIqLock` plus tier config; no scenario data changes beyond audit fixes.
