...
export const PHASE_CAUSAL_CHAINS: Record<HittingPhaseId, CausalChain> = {
  ...
  P3: {
    phase: 'P3',
    trigger: { athlete: 'Hip load and hand load are done. Now the stride is happening.', coach_note: 'Stride window: front foot in flight after hip + hand load are set.' },
    cause: { athlete: 'You land open — toes and chest pointing at the pitcher — instead of sideways.', coach_note: 'Front-foot lands externally rotated with thorax pre-opened; pelvis cannot store rotational torque.' },
    mechanism: { athlete: "Your hips can't store torque, your core can't tension, and your back side either collapses or gets stuck.", coach_note: 'Loss of pelvis-shoulder separation at landing; core fails to reach max tensional state; posterior chain either yields or locks.' },
    result: { athlete: "Late on velocity, can't reach the outside pitch, off-balance at contact, jammed.", coach_note: 'Reduced angular velocity capacity, restricted plate coverage, dynamic balance loss, inside-pitch jam.' },
    fix: { athlete: 'Land sideways with both feet down, chest still toward the plate, back hip controlling the step, core max-tensioned.', coach_note: 'Land with closed thorax + sideways pelvis; back hip dictates stride length; achieve max core tension at front-foot strike.' },
  },
  P4: {
    phase: 'P4',
