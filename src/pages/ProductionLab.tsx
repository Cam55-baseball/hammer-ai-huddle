...
const SKILL_EXERCISES: { [key: string]: Exercise } = {
  ...
  // Bat Speed Development Exercises
  'Bat waggle speed drill': { name: 'Bat waggle speed drill', type: 'skill', description: 'Rapid bat movement in stance before swing - small circles or back-and-forth waggle. Transfers into quick first move and activates fast-twitch muscles.' },
  'Speed swings - 10 reps': { name: 'Speed swings - 10 reps', type: 'skill', description: 'Maximum intent swings focusing purely on bat speed, not contact quality. Swing as fast as possible to train fast-twitch muscle fibers.' },
  'Quick hands drill': { name: 'Quick hands drill', type: 'skill', description: 'Start with bat touching back shoulder, fire hands to contact point as fast as possible. Develops hand path speed and quick trigger.' },
  'Bat behind back rotations': { name: 'Bat behind back rotations', type: 'skill', description: 'Bat placed behind back across shoulder blades. Rotate hips explosively to feel body-generated speed without arm involvement.' },
  'Connection ball drill': { name: 'Connection ball drill', type: 'skill', description: 'Small ball or towel held between lead arm and torso during swing - drops when connected properly. Teaches efficient swing path for maximum speed.' },
  'Speed tee work - 15 swings': { name: 'Speed tee work - 15 swings', type: 'skill', description: 'Tee swings with maximum bat speed intent - quality contact is secondary. Focus purely on generating the fastest possible swing.' },
  'One-handed speed swings': { name: 'One-handed speed swings', type: 'skill', description: 'Single arm swings (lead hand then back hand separately) to develop arm-specific speed and identify weaker hand.' },
  'Wrist snap drill': { name: 'Wrist snap drill', type: 'skill', description: 'Isolate wrist action at contact point with quick snap-through movements. Develops final-phase hand speed through the zone.' },
  'Resistance band speed swings': { name: 'Resistance band speed swings', type: 'skill', description: 'Band attached to bat or around waist, swing against resistance for 5 reps then immediately without for 5 reps. Creates overspeed training effect.' },
  'Swing speed measurement': { name: 'Swing speed measurement', type: 'skill', description: 'Use speed device, app, or video to measure and track swing speed progress. Record baseline and improvements.' },
  // Additional Bat Speed Exercises
  'Velocity ladder swings': { name: 'Velocity ladder swings', type: 'skill', description: '3 swings at 70% intent, 3 at 85%, 3 at 100%. Builds speed progressively and teaches intent differentiation.' },
  'Bat flip speed drill': { name: 'Bat flip speed drill', type: 'skill', description: 'After swing completion, flip bat up with quick wrist snap. Trains fast-twitch wrist action and bat control.' },
  'No-stride speed swings': { name: 'No-stride speed swings', type: 'skill', description: 'Eliminate stride completely, focus purely on rotational speed from set position. Isolates hip and hand speed.' },
  'Fast hands reaction drill': { name: 'Fast hands reaction drill', type: 'skill', description: 'Partner calls "now" at random intervals, fire hands instantly on command. Develops trigger speed and reaction time.' },
  'Exit speed competition': { name: 'Exit speed competition', type: 'skill', description: 'Track exit velocity, compete to beat your personal best with each swing. Creates competitive intent focus.' },
  'Rotational snap drill': { name: 'Rotational snap drill', type: 'skill', description: 'Focus on hip snap speed without full swing. Isolates and maximizes rotational velocity from ground up.' },
  'Light bat max speed - 20 swings': { name: 'Light bat max speed - 20 swings', type: 'skill', description: 'Use training bat 3-5 oz lighter than game bat, swing at absolute maximum speed. Develops fast-twitch patterns.' },
  'Heavy-light contrast - 10 sets': { name: 'Heavy-light contrast - 10 sets', type: 'skill', description: '2 heavy bat swings then 2 light bat swings immediately. Creates overspeed contrast training effect.' },
  'Intent tee work - max effort': { name: 'Intent tee work - max effort', type: 'skill', description: 'Every tee swing at 100% intent - no casual reps allowed. Train nervous system for max output.' },
  'Barrel speed focus drill': { name: 'Barrel speed focus drill', type: 'skill', description: 'Visualize barrel racing to contact point ahead of hands. Mental cue for accelerating bat head speed.' },
};
...
const HITTING_WEEKS: WeekData[] = [
  {
    ...
    days: [
      { day: 'day1', title: 'Day 1: Stance Setup', exercises: ['Athletic stance drill', 'Balance check', 'Grip pressure test', 'Bat waggle speed drill', 'Rotational snap drill', 'No-stride speed swings'].map(getExercise) },
      ...
      { day: 'day3', title: 'Day 3: Tee Work Basics', exercises: ['Center tee - 25 swings', 'Inside tee - 15 swings', 'Outside tee - 15 swings', 'Speed swings - 10 reps', 'Light bat max speed - 20 swings', 'Intent tee work - max effort'].map(getExercise) },
      { day: 'day4', title: 'Day 4: Soft Toss', exercises: ['Front toss - 30 reps', 'Side toss - 20 reps', 'Focus on contact point', 'Quick hands drill', 'Fast hands reaction drill', 'Bat flip speed drill'].map(getExercise) },
  ...
  },
  {
    ...
    days: [
      { day: 'day1', title: 'Day 1: Load Sequence', exercises: ['Negative move drill', 'Stride timing', 'Weight back drill', 'Bat behind back rotations', 'Rotational snap drill', 'Velocity ladder swings'].map(getExercise) },
      ...
      { day: 'day3', title: 'Day 3: Separation', exercises: ['Hip-shoulder separation', 'Rubber band drill', 'Torque creation', 'Connection ball drill', 'No-stride speed swings', 'Quick hands drill'].map(getExercise) },
      { day: 'day4', title: 'Day 4: Live Timing', exercises: ['Front toss timing', 'Machine work', 'Variable speeds', 'Speed tee work - 15 swings', 'Exit speed competition', 'Fast hands reaction drill'].map(getExercise) },
  ...
  },
  {
    ...
    days: [
      { day: 'day1', title: 'Day 1: Hip Mechanics', exercises: ['Hip rotation isolation', 'Ground force drill', 'Core activation', 'One-handed speed swings', 'Rotational snap drill', 'No-stride speed swings'].map(getExercise) },
      ...
      { day: 'day3', title: 'Day 3: Connection', exercises: ['Hip-hand connection', 'Barrel lag drill', 'Sequence work', 'Wrist snap drill', 'Bat flip speed drill', 'Quick hands drill'].map(getExercise) },
      { day: 'day4', title: 'Day 4: Power Application', exercises: ['Weighted bat swings', 'Underload swings', 'Feel the difference', 'Heavy-light contrast - 10 sets', 'Light bat max speed - 20 swings', 'Swing speed measurement'].map(getExercise) },
  ...
  },
  {
    ...
    days: [
      { day: 'day1', title: 'Day 1: Plane Work', exercises: ['Bat path visualization', 'Swing plane drill', 'Level through zone', 'Resistance band speed swings', 'No-stride speed swings', 'Barrel speed focus drill'].map(getExercise) },
      ...
      { day: 'day3', title: 'Day 3: Extension', exercises: ['Extension through ball', 'Follow through work', 'Finish position', 'Speed swings - 10 reps', 'Intent tee work - max effort', 'Velocity ladder swings'].map(getExercise) },
      { day: 'day4', title: 'Day 4: Adjustability', exercises: ['High pitch adjustment', 'Low pitch adjustment', 'In-zone coverage', 'Swing speed measurement', 'Exit speed competition', 'Quick hands drill'].map(getExercise) },
  ...
  },
  {
    ...
    days: [
      { day: 'day1', title: 'Day 1: Bat Speed Drills', exercises: ['Overload/underload protocol', 'Speed chains', 'Intent swings', 'Quick hands drill', 'Heavy-light contrast - 10 sets', 'Velocity ladder swings'].map(getExercise) },
      ...
      { day: 'day3', title: 'Day 3: Strength Transfer', exercises: ['Med ball rotations', 'Power position holds', 'Explosive movements', 'One-handed speed swings', 'Light bat max speed - 20 swings', 'Rotational snap drill'].map(getExercise) },
      { day: 'day4', title: 'Day 4: Game Power', exercises: ['Situational power', 'Driving the ball', 'Hard contact focus', 'Swing speed measurement', 'Exit speed competition', 'Intent tee work - max effort', 'Fast hands reaction drill'].map(getExercise) },
  ...
  },
  {
    ...
    days: [
      { day: 'day1', title: 'Day 1: Approach Work', exercises: ['Count-based approach', 'Pitch selection', 'Zone discipline', 'Bat waggle speed drill', 'Fast hands reaction drill', 'Barrel speed focus drill'].map(getExercise) },
      ...
      { day: 'day3', title: 'Day 3: Velocity Training', exercises: ['High velocity machine', 'Reaction time', 'Quick decisions', 'Speed swings - 10 reps', 'No-stride speed swings', 'Exit speed competition'].map(getExercise) },
      { day: 'day4', title: 'Day 4: Live At-Bats', exercises: ['Simulated game ABs', 'Pressure situations', 'Compete mode', 'Swing speed measurement', 'Intent tee work - max effort', 'Velocity ladder swings'].map(getExercise) },
