// Tissue Cost Scheduler — S2 reliability harness (shared by the property suite,
// the goldens and the full 100k sweep script).

import {
  decide,
  classRank,
} from "../../../supabase/functions/_shared/wic/schedule/tissueCost/decide.ts";
import { TCS_CONFIG } from "../../../supabase/functions/_shared/wic/schedule/tissueCost/config.ts";
import {
  addDays,
  dayDiff,
  fullRestDaysBetween,
} from "../../../supabase/functions/_shared/wic/schedule/tissueCost/tanks.ts";
import type {
  CheckIn,
  DaySchedule,
  Decision,
...
function windowOf(days: DaySchedule[], today: string, anchor?: string) {
  if (!anchor) {
    const history = days.filter((d) => dayDiff(today, d.date) < 0 && dayDiff(today, d.date) >= -28);
