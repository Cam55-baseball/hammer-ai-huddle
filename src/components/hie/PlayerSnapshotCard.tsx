...
import { RefreshCw, TrendingUp, TrendingDown, Minus, Flame, AlertTriangle, CheckCircle2, XCircle, Clock } from 'lucide-react';
...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  inconsistent: { label: '🟡 Inconsistent', color: 'bg-amber-500/10 text-amber-600 border-amber-500/30', icon: <AlertTriangle className="h-4 w-4" /> },
  stalled: { label: '🔴 Stalled', color: 'bg-destructive/10 text-destructive border-destructive/30', icon: <XCircle className="h-4 w-4" /> },
};

export function PlayerSnapshotCard() {
  const { snapshot, isLoading, refreshAnalysis, isRefreshing, isStale } = useHIESnapshot();
    ...
    <Card className="border-2">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-4 flex-wrap">
          ...
          {snapshot.transfer_score != null && (
            <div className="text-center">
              <div className="text-sm font-medium">{snapshot.transfer_score}</div>
              <div className="text-xs text-muted-foreground">Transfer</div>
            </div>
          )}
          {snapshot.decision_speed_index != null && (
            <div className="text-center">
              <div className="text-sm font-medium">{snapshot.decision_speed_index}</div>
              <div className="text-xs text-muted-foreground">Decision Speed</div>
            </div>
          )}
          {snapshot.movement_efficiency_score != null && (
            <div className="text-center">
              <div className="text-sm font-medium">{snapshot.movement_efficiency_score}</div>
        ...
        {snapshot.primary_limiter && (
          <div className="bg-destructive/5 border border-destructive/20 rounded-lg p-3">
            ...
            <div className="text-sm font-semibold text-destructive">{snapshot.primary_limiter}</div>
          </div>
        )}
      </CardContent>
    </Card>
