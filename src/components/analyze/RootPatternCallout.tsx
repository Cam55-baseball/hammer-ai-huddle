 ...
 *
 * Two presentations, same content:
 *  - default: its own bordered card (used where there is no findings card).
 *  - inline:  a plain block, folded into the Key findings card so the report
 *             never repeats the same information twice.
 */
      ...
      {cross.map((g) => {
        ...
        if (inline) {
          return (
            <div
              key={g.pattern.key}
              className="rounded-lg border border-primary/30 bg-primary/5 p-3 sm:p-4"
            >
              {body}
            </div>
          );
