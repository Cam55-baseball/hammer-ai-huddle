...
import { ArrowLeft, CalendarCheck, CalendarX2, ChevronDown, ChevronUp, Flag, HeartPulse, Loader2, PauseCircle, Plane, PlayCircle, Star, Undo2, X } from "lucide-react";
...
export function TellHammersInbox({ checkIn = false, onDone }: { checkIn?: boolean; onDone?: () => void }) {
  ...
  // Only the pain flow holds a pending choice (the face); every other fixed choice sends on tap.
  ...
  function onType(value: string) {
    setText(value); setConfirm(null);
    // Typed text and a tapped choice are alternatives, never combined — except pain, where text is the add-on.
    if (flow !== "pain") setPending(null);
  }
  function prepare() {
    ...
    if (!words) return;
    const prefix: Partial<Record<Flow, string>> = { break: "break ", travel: "travel ", games: "games ", cancelled: "games cancelled ", season: `season ${which ?? "starts"} `, event: `${eventKind ?? "showcase"} `, resume: "back to normal " };
  ...
  const choiceBtn = (key: string, label: string, onClick: () => void, extra = "h-12") =>
    <Button key={key} variant={sel(key) ? "default" : "outline"} aria-pressed={sel(key)} disabled={busy} className={`${extra} whitespace-normal`} data-testid={key} onClick={onClick}>
      {sel(key) && <Loader2 className="mr-1 h-4 w-4 animate-spin" />}{label}
    </Button>;
  ...
  return <section className="rounded-md border border-border bg-card p-3 space-y-3" aria-label={checkIn ? "Anything change?" : "Tell Hammer"}>
    ...
    {(open || checkIn) && <div className="space-y-3">
      ...
      {flow === "cancelled" && <><div className="grid grid-cols-2 gap-2">{[1,7,14,28].map((n) => choiceBtn(`cancel-${n}`, n === 1 ? "Today" : n === 7 ? "This week" : n === 14 ? "Next 2 weeks" : "Next 4 weeks", () => tap(draft("CANCELLED", today, isoShift(today, n - 1)), `cancel-${n}`)))}</div><p className="text-sm">Or tap the days:</p>{pickCalendar(true)}{choiceBtn("cancel-send", "Send these days", () => sendDays("CANCELLED", "cancel-send"), "h-12 w-full")}</>}
      {flow === "pain" && <>{!region ? <><p className="text-sm">Where does it hurt?</p><div className="grid grid-cols-3 gap-2">{REPORT_INJURY_REGIONS.map(r => <Button key={r.key} variant="outline" className="h-12 text-xs" data-testid={`pain-${r.key}`} onClick={() => setRegion(r)}>{r.label}</Button>)}</div></> : <><p className="text-sm">How bad is your {region.label.toLowerCase()}?</p><div className="grid grid-cols-3 gap-2">{FACES.map(f => { const on = pending?.payload.face === f.key; return <Button key={f.key} variant={on ? "default" : "outline"} aria-pressed={on} className="h-20 flex-col" data-testid={`face-${f.key}`} onClick={() => setPending(draft("PAIN",today,today,{ region: region.key, regionLabel: region.label, face: f.key, faceLabel: FACE_LABEL[f.key] }))}><span className="text-2xl">{f.emoji}</span>{FACE_LABEL[f.key]}</Button>; })}</div></> }</>}
      {(flow === "break" || flow === "travel") && <><p className="text-sm">{flow === "travel" ? "How long are you away?" : "How long do you need?"}</p><div className="grid grid-cols-2 gap-2">{(flow === "break" ? [1,3,7] : [1,3,7,14]).map(n => choiceBtn(`hold-${n}`, n === 1 ? "Just today" : n === 7 ? "1 week" : `${n} days`, () => tap(draft("HOLD",today,isoShift(today,n-1),{reason: flow === "travel" ? "travel" : "break"}), `hold-${n}`)))}</div></>}
      {flow === "event" && <>{!eventKind ? <div className="grid grid-cols-2 gap-2">{["Combine","Showcase","Tryout","Camp"].map(k => <Button key={k} variant="outline" onClick={() => setEventKind(k)}>{k}</Button>)}</div> : <><p className="text-sm">Tap the day of the {eventKind.toLowerCase()}.</p>{pickCalendar(false)}{choiceBtn("event-send", "Send this day", () => tap(draft("EVENT",getLocalDateString(picked[0]),getLocalDateString(picked[0]),{label:eventKind,kind:eventKind.toLowerCase()}), "event-send"), "h-12 w-full")}</>}</>}
      {flow === "resume" && <div className="grid grid-cols-1 gap-2">{choiceBtn("resume-today", "Back to normal from today", () => tap(draft("RESUME", today), "resume-today"))}</div>}
      {flow && (flow !== "pain" || region) && <div className="space-y-2">
        ...
        <form onSubmit={e => { e.preventDefault(); prepare(); }} className="space-y-2">
          ...
          {flow === "pain" && !pending && <p className="text-xs text-muted-foreground">Pick a face to send.</p>}
        </form>
