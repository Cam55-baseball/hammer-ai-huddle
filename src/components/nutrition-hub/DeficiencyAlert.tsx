...
export function DeficiencyAlert({ date }: DeficiencyAlertProps) {
  ...
  const { data: alerts } = useQuery({
    ...
    queryFn: async () => {
      ...
      return Object.entries(RDA).map(([key, rda]) => {
        const current = totals[key] || 0;
        const percent = Math.round((current / rda.amount) * 100);
        const level = getLevel(percent);
        return { key, label: rda.label, percent, level, current: Math.round(current * 10) / 10, rda: rda.amount, unit: rda.unit };
      });
    },
    enabled: !!user,
  });
  ...
  const issues = alerts.filter(a => a.level === 'deficient' || a.level === 'low');
      ...
      <CardContent className="p-4 space-y-2">
        ...
        <div className="space-y-1.5">
          {issues.map(item => {
            ...
            return (
              <div key={item.key} className={cn('flex items-center justify-between text-xs rounded-md px-2.5 py-1.5', cfg.bg)}>
