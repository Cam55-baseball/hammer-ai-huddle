...
export function InlineTimer({ label, timer, videoRef, hasVideo, compact = false }: InlineTimerProps) {
  if (compact) {
    return (
    <div className="flex items-center gap-1 rounded-md bg-muted/50 py-0.5 px-1.5 overflow-hidden">
      ...
      <span className="font-mono font-bold text-foreground tracking-wider text-sm shrink-0">
        ...
        {timer.isRunning ? (
          <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0" onClick={timer.stop}>
            <Pause className="h-3 w-3" />
          </Button>
        ) : (
          <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0" onClick={timer.start}>
            <Play className="h-3 w-3" />
          </Button>
        )}
        <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0" onClick={timer.reset}>
          <RotateCcw className="h-3 w-3" />
        </Button>
        {hasVideo && (
          timer.isSynced ? (
            <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0" onClick={timer.unsync}>
              <Unlink className="h-3 w-3" />
            </Button>
          ) : (
            <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0" onClick={() => timer.syncToVideo(videoRef)}>
