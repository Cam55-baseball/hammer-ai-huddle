...
BEGIN
  ...
  v_inv := NULLIF(p_diag->>'global_invariant_status','');
  IF v_inv = 'fatal' AND COALESCE(p_diag->>'validation_status','published') = 'published' THEN
    RAISE EXCEPTION 'global_invariant_failure: refusing to persist published run with fatal invariants';
  END IF;

  v_expected := NULLIF(p_diag->>'expected_snapshot_hash','');
  v_actual   := NULLIF(p_diag->>'snapshot_hash','');
  IF v_expected IS NOT NULL AND v_actual IS NOT NULL AND v_expected <> v_actual THEN
    RAISE EXCEPTION 'snapshot_mutation_detected: expected=% actual=%', v_expected, v_actual;
  END IF;

  v_why_expected := NULLIF(p_diag->>'expected_why_v2_hash','');
  v_why_actual   := NULLIF(p_diag->>'why_v2_hash','');
  IF v_why_expected IS NOT NULL AND v_why_actual IS NOT NULL AND v_why_expected <> v_why_actual THEN
    RAISE EXCEPTION 'why_v2_mutation_detected: expected=% actual=%', v_why_expected, v_why_actual;
  END IF;
  ...
  IF jsonb_typeof(p_rows) = 'array' AND jsonb_array_length(p_rows) > 0 THEN
    ...
    SELECT
      ...
      NULLIF(r->>'distance_feet','')::int,
      NULLIF(r->>'total_reps','')::int,
      NULLIF(r->>'dosage_unit',''),
      ...
      -- Execution layer: pure passthrough. No defaults, no coalesce, no
      -- derivation. Absent key -> NULL, exactly as before this change.
      (r->>'intent_tag'),
      (r->>'execution_note'),
      (r->>'per_side')::boolean,
      (r->>'asymmetry_rule'),
      (r->>'open_ended')::boolean,
      (r->>'set_range_max')::int,
      (r->>'density_target_seconds')::int,
      (r->>'rir_low')::int,
      (r->>'rir_high')::int,
      CASE WHEN jsonb_typeof(r->'cue_ids') = 'array'
           THEN ARRAY(SELECT jsonb_array_elements_text(r->'cue_ids'))
           ELSE NULL END,
      (r->>'troubleshoot_video_id')::uuid,
      (r->>'intensity_mode')
  ...
  INSERT INTO public.wk_generation_diagnostics (
    ...
    determinism_seed, determinism_trace, engine_execution_order,
    global_validator_status, snapshot_hash, snapshot_integrity_status,
    governance_catalog_hash, why_v2_completeness_score,
    system_state, system_state_hash, engine_signature_hashes,
    why_v2_hash, validator_aggregate_hash, global_invariant_status
  ) VALUES (
    p_user, p_date,
    ...
    NULLIF(p_diag->>'determinism_seed',''),
    NULLIF(p_diag->'determinism_trace','null'::jsonb),
    CASE WHEN jsonb_typeof(p_diag->'engine_execution_order') = 'array'
         THEN ARRAY(SELECT jsonb_array_elements_text(p_diag->'engine_execution_order'))
         ELSE NULL END,
    NULLIF(p_diag->>'global_validator_status',''),
    NULLIF(p_diag->>'snapshot_hash',''),
    NULLIF(p_diag->>'snapshot_integrity_status',''),
    NULLIF(p_diag->>'governance_catalog_hash',''),
    NULLIF(p_diag->>'why_v2_completeness_score','')::int,
    NULLIF(p_diag->'system_state','null'::jsonb),
    NULLIF(p_diag->>'system_state_hash',''),
    NULLIF(p_diag->'engine_signature_hashes','null'::jsonb),
    NULLIF(p_diag->>'why_v2_hash',''),
    NULLIF(p_diag->>'validator_aggregate_hash',''),
    NULLIF(p_diag->>'global_invariant_status','')
