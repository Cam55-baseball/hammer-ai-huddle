...
-- 1. Replace insert_training_block_atomic with DO NOTHING + fallback SELECT pattern
...
BEGIN
  -- Enforce snapshot isolation to prevent phantom reads
  ...
  IF p_idempotency_key IS NOT NULL THEN
    -- Attempt insert without write amplification
    ...
    -- Conflict path: fetch and lock existing row
...
-- 3. Prevent duplicate scheduled dates per block
