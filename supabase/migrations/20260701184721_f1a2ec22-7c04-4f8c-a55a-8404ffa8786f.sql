-- Phase 11–12 — E2E Unification & Production Lock (additive)

ALTER TABLE public.wk_generation_diagnostics
  ADD COLUMN IF NOT EXISTS determinism_seed text,
  ADD COLUMN IF NOT EXISTS determinism_trace jsonb,
  ADD COLUMN IF NOT EXISTS engine_execution_order text[],
  ADD COLUMN IF NOT EXISTS global_validator_status text,
  ADD COLUMN IF NOT EXISTS snapshot_hash text,
  ADD COLUMN IF NOT EXISTS snapshot_integrity_status text,
  ADD COLUMN IF NOT EXISTS governance_catalog_hash text,
  ADD COLUMN IF NOT EXISTS why_v2_completeness_score int;

-- Extend the atomic persistence RPC. Signature preserved. Snapshot immutability
-- enforced: if p_diag contains snapshot_hash and expected_snapshot_hash and they
-- diverge, abort with a fatal error.
