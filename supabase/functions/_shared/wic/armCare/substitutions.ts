// Phase 10 — Arm Care Substitution Ladders

export interface ArmCareCatalogEntry {
  slug: string;
  name: string;
...
export interface ArmCareSubstitutionLadder {
...
const EMPTY: ArmCareSubstitutionLadder = {
...
export function resolveArmCareSubstitutionLadder(input: {
  movement: ArmCareCatalogEntry;
  catalog: readonly ArmCareCatalogEntry[];
  availableEquipment?: readonly string[];
  environment?: "indoor" | "outdoor" | string;
  phase?: string;
  trainingAgeClass?: string;
}): ArmCareSubstitutionLadder {
  ...
  const seasonLegal = (c: ArmCareCatalogEntry) => !phase || !c.season_legality || c.season_legality[phase] !== false;
  const ageLegal = (c: ArmCareCatalogEntry) => !trainingAgeClass || !c.training_age_legality || c.training_age_legality[trainingAgeClass] !== false;
  const equipmentFits = (c: ArmCareCatalogEntry) => {
...
export function armCareLadderCompleteness(l: ArmCareSubstitutionLadder, familySize: number): { complete: boolean; score: number } {
