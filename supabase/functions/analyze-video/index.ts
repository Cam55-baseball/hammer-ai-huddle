...
${hasHistory ? `Based on the historical data above and this current analysis, generate "The Scorecard" progress report comparing current performance to ALL previous uploads.` : `This is the player's first analysis - establish a baseline.`}`
    ...
    // Call AI (Google AI Studio direct, Lovable Gateway fallback) with tool-calling for structured output
    ...
    if (!aiResult.ok) {
      const errorText = aiResult.errorBody || "";
      console.error("AI provider error:", aiResult.provider, aiResult.status, errorText);

      const errorData = {
        code: aiResult.status.toString(),
        message: errorText,
      ...
      if (aiResult.status === 429) {
        throw new Error(
          `ai_gateway_rate_limited: provider=${aiResult.provider} upstream_status=429 body=${truncatedBody}`
        );
      }
      if (aiResult.status === 402) {
        throw new Error(
          `ai_gateway_payment_required: provider=${aiResult.provider} upstream_status=402 body=${truncatedBody}`
        );
      }
      throw new Error(
        `ai_gateway_error: provider=${aiResult.provider} upstream_status=${aiResult.status} body=${truncatedBody}`
      );
    }

    const data = aiResult.data;
    console.log(`[ANALYZE-VIDEO] AI provider=${aiResult.provider}`);
