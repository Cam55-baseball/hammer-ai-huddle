...
## In progress
- [x] Today-plan input integrity (browser click-through still unverified — test sign-in unavailable): chat equipment must persist (deterministic parse → save_equipment_context → confirmed-only confirmation); no card may render as a request for information; leaked slugs/tokens removed from athlete copy; game-day defense renders a position-specific primer with override; end-to-end verification in an authenticated session.
