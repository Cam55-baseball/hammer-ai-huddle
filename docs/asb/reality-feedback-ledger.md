...
| RFL-079 | Display format mixing not declared per category — 1–10 vs pass/fail vs % vs degrees vs timing. | med | Open — awaiting user ratification | Audit §2 contains per-row proposed display format table with ratification checkboxes. User must mark each row before Phase F UI is built. |
